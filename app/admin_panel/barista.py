"""
Module contains admin view for Barista model
"""
...
class BaristaAdmin(ModelView):
    ...
    form_args = dict(
        ...
        roles=dict(
            validators=[
                DataRequired(message=gettext("Выберите уровень доступа сотрудника"))
            ]
        ),
        password=dict(validators=[DataRequired()]),
    )
    form_widget_args = {
        "name": {"placeholder": gettext("Имя сотрудника")},
        "phone_number": {"placeholder": gettext("Мобильный телефон")},
        "email": {"placeholder": gettext("Емейл")},
        "password": {"placeholder": gettext("Пароль для входа")},
        "shop": {"placeholder": gettext("Выберите места работы сотрудника")},
        "roles": {"placeholder": gettext("Выберите уровень доступа для сотрудника")},
        "confirmed_at": {"data-date-format": "DD.MM.YYYY HH:mm"},
    }

    @property
    def can_edit(self):
        """Edit operation, depends on role"""
